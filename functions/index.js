...
exports.createFiles = functions.https.onRequest(async (req, res) => {
  ...
  const events = snapshot.docs.map((rec) => {
    return {
      ...rec.data(),
      id: rec.id,
    };
  ...
  for (const event of events) {
    ...
    try {
      const file = await driveCreatePromise({
...
function writePromise(requestWithoutAuth) {
  return new Promise((resolve, reject) => {
    return getAuthorizedClient().then((client) => {
      const sheets = google.sheets('v4');
      const request = requestWithoutAuth;
      request.auth = client;
      return sheets.spreadsheets.values.update(request, (err, response) => {
        if (err) {
...
function driveCreatePromise(requestWithoutAuth) {
  return new Promise((resolve, reject) => {
    return getAuthorizedClient().then((client) => {
      const drive = google.drive('v3');
      const request = requestWithoutAuth;
      request.auth = client;
      return drive.files.update(request, function (err, response) {
        if (err) {
          functions.logger.log(`The API returned an error: ${err}`);
          return reject(err);
        }
